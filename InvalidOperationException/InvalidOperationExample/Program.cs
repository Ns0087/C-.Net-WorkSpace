...
namespace InvalidOperationExceptionExample
{
    class BankAccount
    {
        ...
        public void Transfer(BankAccount sourceAccount, BankAccount destinationAccount, double amount)
        {
            ...
            try
            {
                ...
                if (amount > sourceAccount.CurrentBalance)
                    throw new InvalidOperationException($"Insufficient balance in the source account. The current balance is {sourceAccount.CurrentBalance}. But the amount to transfer is {amount}");
